...
class Spider(Spider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        result = {}
        url = f'https://jpttavmovtv7.cc/tag_list?tid={tid}&idx={pg}'
        try:
            rsp = self.fetch(url)
    ...
    def detailContent(self, array):
        tid = array[0]
        url = tid if tid.startswith('http') else f'https://jpttavmovtv7.cc/{tid}'
        try:
            rsp = self.fetch(url)
            root = etree.HTML(rsp.text)
                ...
                if matches:
                    for match in matches:
                        if match.startswith('//'):
                            ...
                            return 'https://jpttavmovtv7.cc/' + match
            ...
            for pattern in js_patterns:
                ...
                if match:
                    ...
                    if video_url.startswith('//'):
                        ...
                        return 'https://jpttavmovtv7.cc/' + video_url
    ...
    def searchContent(self, key, quick, pg="1"):
        result = {}
        url = f'https://jpttavmovtv7.cc/search?kw={urllib.parse.quote(key)}'
        try:
            rsp = self.fetch(url)
            root = etree.HTML(rsp.text)
            videos = root.xpath('//div[contains(@class,"oneVideo")]')
        ...
        if flag == "注意身体":
            try:
                if id.startswith('http') and '.m3u8' in id:
                    ...
                    url = id if id.startswith('http') else f'https://jpttavmovtv7.cc/{id}'
                    rsp = self.fetch(url)
                    play_url = self.extractVideoUrl(rsp.text)
                    result["parse"] = 0
                    result["playUrl"] = ''
            ...
            result["header"] = {
                ...
                "Referer": "https://jpttavmovtv7.cc/",
                "Origin": "https://jpttavmovtv7.cc/"
            }
        return result
